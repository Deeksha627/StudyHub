  ...
  const Navbar = () => {
    ...
    return (
      <nav className="fixed top-2 left-1/2 transform -translate-x-1/2 z-50 w-[calc(100%-0.5rem)] sm:w-[calc(100%-1rem)] max-w-6xl px-2 sm:px-3 font-poppins">
        <motion.div
          className={`transition-all duration-300 rounded-full mx-auto w-full max-w-[90rem] ${
            scrolled
              ...
              {navItems.map((item, index) => {
                ...
                return (
                  <button
                    ...
                    className={`relative z-10 px-2 sm:px-3 py-1 sm:py-1.5 rounded-full text-xs sm:text-sm font-medium transition-colors duration-200 flex items-center space-x-1 ${
                  ...
                  >
                    <Icon
                      className={`w-3 h-3 sm:w-4 sm:h-4 ${
                        activeItem === item.name ? 'text-white' : 'text-blue-600 dark:text-blue-400'
                      }`}
                    />
                    <span>{item.name}</span>
                  </button>
